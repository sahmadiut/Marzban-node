...
1. کانفیگ از پنل مرزبان دریافت می‌شود (شامل inbounds، routing، outbounds)
2. API inbound و routing rules مربوطه توسط سرویس اضافه می‌شوند
3. اگر فایل کانفیگ کاستوم وجود داشته باشد، routing و outbounds آن جایگزین می‌شوند
4. routing rule مربوط به API همیشه در اولویت اول حفظ می‌شود
...
- قوانین routing مربوط به API نیز به صورت خودکار اضافه می‌شوند و **همیشه در اولویت اول قرار می‌گیرند**
- نیازی به تنظیم دستی API در کانفیگ سفارشی نیست
- حتی اگر routing کاستوم داشته باشید، routing rule مربوط به API حفظ می‌شود
